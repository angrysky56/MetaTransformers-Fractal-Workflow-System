...
def visualize_clusters(graph, step):
    # Detect clusters
    ...
    for i, comm in enumerate(communities):
        nx.draw_networkx_nodes(graph, pos, nodelist=list(comm), node_color=colors[i % len(colors)])
    nx.draw_networkx_edges(graph, pos)
    plt.title(f"Clusters at Step {step}")
    plt.show()

# Example use:
visualize_clusters(net.graph, step=10)
